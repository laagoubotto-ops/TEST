...
def _serialize_rate(rr: RoomRate) -> dict:
    return {
        ...
        "rate_sgl": float(rr.rate_sgl) if rr.rate_sgl is not None else 0,
        "rate_dbl": float(rr.rate_dbl) if rr.rate_dbl is not None else 0,
        "rate_tpl": float(rr.rate_tpl) if rr.rate_tpl is not None else None,
